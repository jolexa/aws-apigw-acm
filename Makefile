...
deploy-apigw: prep
	aws cloudformation deploy \
		...
		--stack-name aws-apigw-acm-reference \
		--region $(REGION) \
...
deploy-acm: deploy-apigw
	...
	aws cloudformation deploy \
		...
		--stack-name aws-apigw-acm-reference-acm-certs \
		--region us-east-1 \
...
deploy-domainname: deploy-acm
	...
	scripts/create_domain_name.py \
		--cert-arn $(shell aws cloudformation --region us-east-1 describe-stacks --stack-name aws-apigw-acm-reference-acm-certs --query Stacks[0].Outputs[0].OutputValue --output text) \
		--domain-name $(URL) --region $(REGION) || exit 0

deploy: deploy-domainname
	aws cloudformation deploy \
		--template-file post_domain.yml \
		--stack-name aws-apigw-acm-reference-post \
		--region $(REGION) \
